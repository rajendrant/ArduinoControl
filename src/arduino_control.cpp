...
resp = Message_init_zero;
resp.which_msg = Message_this_address_tag;
resp.msg.this_address = this_address;
}

void handle_system_uptime() {
resp = Message_init_zero;
resp.which_msg = Message_system_uptime_tag;
resp.msg.system_uptime = millis();
}
...
bool ArduinoControlClass::process_message(const uint8_t *req, uint8_t req_len, uint8_t **resp, uint8_t *resp_len) {
...
break;
case Message_this_address_tag:
handle_this_address(this_address);
break;
case Message_system_uptime_tag:
